    ...
    IConfigurable<SystemEventBoostServiceConfig>
{
    ...
    protected override Task OnStart()
    {
        ...
        //核心硬规则：按开关动态生成，只提示已开启的模式
        StringBuilder hard = new();
        hard.AppendLine("你拥有「主动事件增强」能力：可自主创建定时任务、切换报点模式、调整活跃节奏、开关各种陪伴模式，以及进入「工作模式」像专业 Agent 一样执行任务。");
        hard.AppendLine("当主人用自然语言表达对应需求时，应当主动调用本服务的对应函数响应，让主人感到贴心。");
        if (Configuration.MasterGameMode)
            hard.AppendLine("- 「进入游戏模式/陪我打游戏」→ 进入游戏陪伴模式");
        if (Configuration.MasterSleepMode)
            hard.AppendLine("- 「我要睡觉了/晚安/明天X点叫我」→ 进入睡眠模式");
        if (Configuration.MasterDndMode)
            hard.AppendLine("- 「别吵我/勿扰/安静点」→ 开启勿扰模式");
        if (Configuration.MasterCuteMode)
            hard.AppendLine("- 「开启撒娇」→ 开启撒娇模式");
        if (Configuration.MasterWorkMode)
            hard.AppendLine("- 主人下达明确的工作任务（写代码、改文件、跑脚本、查资料、做分析、处理数据、操作电脑、多步任务等）时，必须主动调用 <EnterWorkMode/> 进入工作模式：先输出 <plan> 编号计划，然后系统会引导你逐步执行，每完成一步调用 <WorkStepDone/> 推进，直到全部完成。不要跳过工作模式直接零散地执行。");
        string hardRules = hard.ToString();

        //详细规则：按开关过滤，只保留已开启模式的说明
        StringBuilder detail = new();
        if (Configuration.MasterScheduledTask)
            detail.AppendLine("- 创建循环或临时的定时任务（早晚问候、定时提醒、让自己定时做某事）");
        if (Configuration.MasterGameMode)
            detail.AppendLine("- 进入游戏陪伴模式：固定间隔主动用深度视觉查看屏幕游戏画面，鼓励或建议主人");
        if (Configuration.MasterSleepMode)
            detail.AppendLine("- 进入睡眠模式：设定唤醒时间或等主人发消息后再恢复活跃");
        if (Configuration.MasterDndMode)
            detail.AppendLine("- 开启勿扰模式：自主活动但不打扰主人；或开启撒娇模式：更粘人主动找主人");
        if (Configuration.MasterPeakMode)
            detail.AppendLine("- 控制峰谷模式（高峰时段停止自主活跃）以及自己的活跃间隔");
        if (Configuration.MasterReportMode)
            detail.AppendLine("- 报点模式：可创建/切换自定义报点提示词（如「学习模式」「新闻模式」），报点间隔算法不变");
        if (Configuration.MasterWorkMode)
            detail.AppendLine("- 工作模式：主人下达具体工作任务时，用 <EnterWorkMode/> 进入，先列 <plan> 计划再逐步执行，每步完成用 <WorkStepDone/> 推进；可暂停/恢复/中止");
    ...
    static List<string> ParsePlan(string output)
    {
        ...
        foreach (string rawLine in body.Split('\n'))
        {
            ...
            //去掉编号前缀：1. 2、3) 4- (5) 或 -、• 项目符号
            string cleaned = System.Text.RegularExpressions.Regex.Replace(line, @"^\s*(\d+[\.、\)\]\-]\s*|\-\s*|•\s*|\*\s*)", "");
    ...
    {
        if (Configuration.WorkModeEnabled == false)
        {
